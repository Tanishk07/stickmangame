...
public class HatBoyAnimation extends CharacterAnimationBase {
    ...
    HatBoyAnimation(double xPos,double yPos,double xFactor,double yFactor,double areaFactor){
        ...
        idleImagesChar= new Image[10];
        runImagesChar = new Image[8];
        walkImagesChar= new Image[8];

        for(int i=1;i<=10;i++){
            idleImagesChar[i-1]=new Image("Character/Hat_Character/Idle ("+i+").png");
        }
        for(int i=1;i<=8;i++){
            runImagesChar[i-1]=new Image("Character/Hat_Character/Run ("+i+").png");
        }
        for(int i=1;i<=8;i++){
            walkImagesChar[i-1]=new Image("Character/Hat_Character/Run ("+i+").png");
        }
    ...
    public void createRunAnimation(double xEnd, double gap,double stickLength, Timeline backgroundTimeline, EventHandler<ActionEvent> onAnimationComplete, EventHandler<ActionEvent> onEvent, CherryAndBee cb) {
        ...
        timeline[0] = new Timeline(
                new KeyFrame(duration, e -> {
                    ...
                    if (reverse) {
                        this.setLayoutY(this.yPos + 167 * yFactor);
                    ...
                    if ((mutableXStart[0] > stickLength + this.xStart && stickLength+96*xFactor<gap-this.xStart) || (reverse && mutableXStart[0] >= gap)||((mutableXStart[0] > stickLength + this.xStart && stickLength+96*xFactor>xEnd-xStart))) {
                    ...
                    if (cb.isCherryStatus()) {
                        if (splitBoundIntersect(this.getBoundsInParent(),0.8 ,0.6,cb.getCherryBound())) {
                            onEvent.handle(new CustomActionEvent(this, this, 1));

                        }
                    }
                    if (cb.isBeeStatus()) {
                        if (splitBoundIntersect(this.getBoundsInParent(),0.8 ,0.6,cb.getBeeBound())) {
                    ...
                    if (mutableXStart[0] >= mutableXEnd[0] - 75 * xFactor  && (stickLength+96*xFactor<xEnd-xStart)) {
                        timeline[0].stop();
                        backgroundTimeline.stop();
                        this.setLayoutY(this.yPos);
                        runImageView.setOpacity(0);
