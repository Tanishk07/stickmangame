...
public class BoyAnimation extends CharacterAnimationBase {
    ...
    BoyAnimation(double xPos, double yPos, double xFactor, double yFactor, double areaFactor) {
        ...
        idleImagesChar = new Image[15];
        runImagesChar = new Image[15];
        walkImagesChar = new Image[15];

        for (int i = 1; i <= 15; i++) {
            idleImagesChar[i - 1] = new Image("Character/Boy_Character/Idle (" + i + ").png");
        }
        for (int i = 1; i <= 15; i++) {
            runImagesChar[i - 1] = new Image("Character/Boy_Character/Run (" + i + ").png");
        }
        for (int i = 1; i <= 15; i++) {
            walkImagesChar[i - 1] = new Image("Character/Boy_Character/Walk (" + i + ").png");
        }

        this.getChildren().addAll(idleImageView, runImageView, walkImageView);
        this.width = 110 * xFactor;
        this.height = 50 * yFactor;
    ...
    public void createRunAnimation(double xEnd, double gap,double stickLength, Timeline backgroundTimeline, EventHandler<ActionEvent> onAnimationComplete, EventHandler<ActionEvent> onEvent, CherryAndBee cb) {
        ...
        timeline[0] = new Timeline(
                new KeyFrame(duration, e -> {
                    ...
                    if (reverse) {
                        this.setLayoutY(this.yPos + 175 * yFactor);
                    ...
                    if ((mutableXStart[0] > stickLength + this.xStart && stickLength+50*xFactor<gap-this.xStart) || (reverse && mutableXStart[0] >= gap)||((mutableXStart[0] > stickLength + this.xStart && stickLength+50*xFactor>xEnd-xStart))) {
                    ...
                    if (cb.isCherryStatus()) {
                        if (splitBoundIntersect(this.getBoundsInParent(),0.5 ,0.1,cb.getCherryBound())) {
                            onEvent.handle(new CustomActionEvent(this, this, 1));

                        }
                    }
                    if (cb.isBeeStatus()) {
                        if (splitBoundIntersect(this.getBoundsInParent(),0.5 ,0.1,cb.getBeeBound())) {
                    ...
                    if (mutableXStart[0] >= mutableXEnd[0] - 30 * xFactor  && (stickLength+50*xFactor<xEnd-xStart)) {
                        timeline[0].stop();
                        backgroundTimeline.stop();
                        this.setLayoutY(this.yPos);
                        runImageView.setOpacity(0);
